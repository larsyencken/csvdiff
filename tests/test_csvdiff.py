...
class TestCsvdiff(unittest.TestCase):
    ...
    def csvdiff_summary_cmd(self, *args, **kwargs):
        ...
        if "ignore_columns" in kwargs:
            base_args = ('--output', t.name, '--style', 'summary', '--ignore_columns',
                         kwargs['ignore_columns'])
        else:
            base_args = ('--output', t.name, '--style', 'summary')

        result = self.runner.invoke(csvdiff.csvdiff_cmd, base_args + args)
    ...
    def test_diff_with_valid_ignore(self):
        """
            If you pass the diff command a list of valid columns (ones that exist in the files)
            to ignore then these columns should be ignored when the diff is carried out. This
            test will use the same diff tests as above, but will ignore the columns that have
